...
int  names[26], anymarks, nbra, subnewa, fchange, wrapp;
...
void commands(void) {
  ...
  for (;;) {
    ...
    case 't':  setnoaddr(); newline();quit(0);
    caseread: if ((io = open((const char*)file, 0)) < 0) { lastc = '\n';  error(file); }  setwide();  squeeze(0);
                 ninbuf = 0;  c = zero != dol;
...
char* getline_blk(unsigned int tl) {
   char *bp, *lp;  int nl;  lp = linebuf;  bp = getblock(tl, READ);
  nl = nleft;
  tl &= ~((BLKSIZE/2)-1);
...
void readfile(const char* c) {setnoaddr(); if (vflag && fchange) { fchange = 0; /* error(Q); */ } filename(c); init();
